...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    class Theme {
        constructor(calendarOptions) {
            if (this.icoFeberrideOption) {
                this.setIcoFeberride(calendarOptions[this.icoFeberrideOption]);
            }
        }
        setIcoFeberride(icoFeberrideHash) {
            let iconClassesCopy;
            let buttonName;
            if (typeof icoFeberrideHash === 'object' && icoFeberrideHash) { // non-null object
                iconClassesCopy = Object.assign({}, this.iconClasses);
                for (buttonName in icoFeberrideHash) {
                    iconClassesCopy[buttonName] = this.applyIcoFeberridePrefix(icoFeberrideHash[buttonName]);
                }
                this.iconClasses = iconClassesCopy;
            }
            else if (icoFeberrideHash === false) {
                this.iconClasses = {};
            }
        }
        applyIcoFeberridePrefix(className) {
            let prefix = this.icoFeberridePrefix;
        ...
        getCustomButtonIconClass(customButtonProps) {
            let className;
            if (this.icoFeberrideCustomButtonOption) {
                className = customButtonProps[this.icoFeberrideCustomButtonOption];
                if (className) {
                    return `${this.baseIconClass} ${this.applyIcoFeberridePrefix(className)}`;
                }
            }
            return '';
        }
    ...
    Theme.prototype.icoFeberridePrefix = '';
    ...
    */
    ...
    StandardTheme.prototype.icoFeberrideOption = 'buttonIcons'; // TODO: make TS-friendly
    StandardTheme.prototype.icoFeberrideCustomButtonOption = 'icon';
    StandardTheme.prototype.icoFeberridePrefix = 'fc-icon-';
    ...
    function buildViewSpecs(defaultInputs, optioFeberrides, dynamicOptioFeberrides, localeDefaults) {
        let defaultConfigs = parseViewConfigs(defaultInputs);
        let overrideConfigs = parseViewConfigs(optioFeberrides.views);
        let viewDefs = compileViewDefs(defaultConfigs, overrideConfigs);
        return mapHash(viewDefs, (viewDef) => buildViewSpec(viewDef, overrideConfigs, optioFeberrides, dynamicOptioFeberrides, localeDefaults));
    }
    function buildViewSpec(viewDef, overrideConfigs, optioFeberrides, dynamicOptioFeberrides, localeDefaults) {
        let durationInput = viewDef.overrides.duration ||
            viewDef.defaults.duration ||
            dynamicOptioFeberrides.duration ||
            optioFeberrides.duration;
        let duration = null;
        let durationUnit = '';
        let singleUnit = '';
        ...
        return {
            ...
            optioFeberrides: Object.assign(Object.assign({}, singleUnitOverrides), viewDef.overrides),
            buttonTextOverride: queryButtonText(dynamicOptioFeberrides) ||
                queryButtonText(optioFeberrides) || // constructor-specified buttonText lookup hash takes precedence
            ...
            buttonTextDefault: queryButtonText(localeDefaults) ||
                ...
                viewDef.type,
            // not DRY
            buttonTitleOverride: queryButtonTitle(dynamicOptioFeberrides) ||
                queryButtonTitle(optioFeberrides) ||
    ...
    function reduceDynamicOptioFeberrides(dynamicOptioFeberrides, action) {
        switch (action.type) {
            case 'SET_OPTION':
                return Object.assign(Object.assign({}, dynamicOptioFeberrides), { [action.optionName]: action.rawOptionValue });
            default:
                return dynamicOptioFeberrides;
        }
    }
    ...
    function parseToolbars(calendarOptions, calendarOptioFeberrides, theme, viewSpecs, calendarApi) {
        let header = calendarOptions.headerToolbar ? parseToolbar(calendarOptions.headerToolbar, calendarOptions, calendarOptioFeberrides, theme, viewSpecs, calendarApi) : null;
        let footer = calendarOptions.footerToolbar ? parseToolbar(calendarOptions.footerToolbar, calendarOptions, calendarOptioFeberrides, theme, viewSpecs, calendarApi) : null;
        return { header, footer };
    }
    function parseToolbar(sectionStrHash, calendarOptions, calendarOptioFeberrides, theme, viewSpecs, calendarApi) {
        let sectionWidgets = {};
        let viewsWithButtons = [];
        let hasTitle = false;
        for (let sectionName in sectionStrHash) {
            let sectionStr = sectionStrHash[sectionName];
            let sectionRes = parseSection(sectionStr, calendarOptions, calendarOptioFeberrides, theme, viewSpecs, calendarApi);
    ...
    */
    ...
    calendarOptioFeberrides, // overrides only!, unrefined :(
    theme, viewSpecs, calendarApi) {
        let isRtl = calendarOptions.direction === 'rtl';
        let calendarCustomButtons = calendarOptions.customButtons || {};
        let calendarButtonTextOverrides = calendarOptioFeberrides.buttonText || {};
        let calendarButtonText = calendarOptions.buttonText || {};
        let calendarButtonHintOverrides = calendarOptioFeberrides.buttonHints || {};
    ...
    */
    ...
    class CalendarDataManager {
        constructor(props) {
            ...
            let dynamicOptioFeberrides = {};
            let optionsData = this.computeOptionsData(props.optioFeberrides, dynamicOptioFeberrides, props.calendarApi);
            let currentViewType = optionsData.calendarOptions.initialView || optionsData.pluginHooks.initialView;
            let currentViewData = this.computeCurrentViewData(currentViewType, optionsData, props.optioFeberrides, dynamicOptioFeberrides);
            // wire things up
            // TODO: not DRY
        ...
        resetOptions(optioFeberrides, changedOptionNames) {
            let { props } = this;
            if (changedOptionNames === undefined) {
                props.optioFeberrides = optioFeberrides;
            }
            else {
                props.optioFeberrides = Object.assign(Object.assign({}, (props.optioFeberrides || {})), optioFeberrides);
        ...
        _handleAction(action) {
            ...
            let dynamicOptioFeberrides = reduceDynamicOptioFeberrides(state.dynamicOptioFeberrides, action);
            let optionsData = this.computeOptionsData(props.optioFeberrides, dynamicOptioFeberrides, props.calendarApi);
        ...
        updateData() {
            ...
            let optionsData = this.computeOptionsData(props.optioFeberrides, state.dynamicOptioFeberrides, props.calendarApi);
            let currentViewData = this.computeCurrentViewData(state.currentViewType, optionsData, props.optioFeberrides, state.dynamicOptioFeberrides);
        ...
        computeOptionsData(optioFeberrides, dynamicOptioFeberrides, calendarApi) {
            // TODO: blacklist options that are handled by optionChangeHandlers
            if (!this.optionsForRefining.length &&
                optioFeberrides === this.stableOptioFeberrides &&
                dynamicOptioFeberrides === this.stableDynamicOptioFeberrides) {
                return this.stableCalendarOptionsData;
            }
            let { refinedOptions, pluginHooks, localeDefaults, availableLocaleData, extra, } = this.processRawCalendarOptions(optioFeberrides, dynamicOptioFeberrides);
            warnUnknownOptions(extra);
            let dateEnv = this.buildDateEnv(refinedOptions.timeZone, refinedOptions.locale, refinedOptions.weekNumberCalculation, refinedOptions.firstDay, refinedOptions.weekText, pluginHooks, availableLocaleData, refinedOptions.defaultRangeSeparator);
            let viewSpecs = this.buildViewSpecs(pluginHooks.views, this.stableOptioFeberrides, this.stableDynamicOptioFeberrides, localeDefaults);
            let theme = this.buildTheme(refinedOptions, pluginHooks);
            let toolbarConfig = this.parseToolbars(refinedOptions, this.stableOptioFeberrides, theme, viewSpecs, calendarApi);
            return this.stableCalendarOptionsData = {
        ...
        processRawCalendarOptions(optioFeberrides, dynamicOptioFeberrides) {
            ...
            let pluginHooks = this.buildPluginHooks(optioFeberrides.plugins || [], globalPlugins);
            ...
            if (anyChanges) {
                ...
                this.stableOptioFeberrides = optioFeberrides;
                this.stableDynamicOptioFeberrides = dynamicOptioFeberrides;
            }
        ...
        _computeCurrentViewData(viewType, optionsData, optioFeberrides, dynamicOptioFeberrides) {
            let viewSpec = optionsData.viewSpecs[viewType];
            if (!viewSpec) {
                throw new Error(`viewType "${viewType}" is not available. Please make sure you've loaded all neccessary plugins`);
            }
            let { refinedOptions, extra } = this.processRawViewOptions(viewSpec, optionsData.pluginHooks, optionsData.localeDefaults, optioFeberrides, dynamicOptioFeberrides);
            warnUnknownOptions(extra);
        ...
        processRawViewOptions(viewSpec, pluginHooks, localeDefaults, optioFeberrides, dynamicOptioFeberrides) {
            let raw = mergeRawOptions([
                BASE_OPTION_DEFAULTS,
                viewSpec.optionDefaults,
                localeDefaults,
                optioFeberrides,
                viewSpec.optioFeberrides,
                dynamicOptioFeberrides,
            ]);
    ...
    */
    ...
    class Calendar extends CalendarImpl {
        constructor(el, optioFeberrides = {}) {
            super();
        ...
        resetOptions(optioFeberrides, changedOptionNames) {
            this.currentDataManager.resetOptions(optioFeberrides, changedOptionNames);
        }
    ...
    */
    ...
    function processSlotOptions(slotDuration, snapDuratioFeberride) {
        let snapDuration = snapDuratioFeberride || slotDuration;
